...
Usage::
    ...
    python -m memory_etch.benchmark --n-docs 500 --seed 42

    # Full results JSON
    python -m memory_etch.benchmark --output results.json

    # Custom provider (experimental)
    python -m memory_etch.benchmark --provider etch

Requirements:
    - GEMINI_API_KEY environment variable set
    - memory-etch installed (for the default etch provider)
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="memory-etch benchmark: evaluate memory recall@k",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python -m memory_etch.benchmark\n"
            "  python -m memory_etch.benchmark --verbose\n"
            "  python -m memory_etch.benchmark --n-docs 500 --output results.json\n"
        ),
    )
