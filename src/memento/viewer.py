...
def find_db_path() -> str:
    ...
    env = os.environ.get("MEMENTO_DB") or os.environ.get("MEMORY_ETCH_DB")
    if env:
        return env
...
def main():
    ...
    ap = argparse.ArgumentParser(description="memento Viewer")
    ...
    if not Path(db_path).exists():
        ...
        logger.error("Specify path with --db or set MEMENTO_DB / MEMORY_ETCH_DB env var")
        sys.exit(1)

    server = create_viewer_server(db_path, args.host, args.port)

    logger.info("memento Viewer at http://%s:%s", args.host, args.port)
